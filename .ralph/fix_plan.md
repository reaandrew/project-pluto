...
- [~] **1.2** Discovery providers in `lambdas/discover/providers/`: `companieshouse`, `googleplaces`, `csv`. Each returns normalized `DiscoveredBusiness`. *(in flight 2026-05-11: shared `lambdas/pkg/discovery/` package landed first with `DiscoveredBusiness` struct + `Provider` interface (`ID()` / `CostPerCallUSD()` / `Find(ctx, FindRequest, cap)`) + `FindRequest` (subset of TargetingProfile to avoid the import cycle pkg/targeting would create). CSV provider shipped: `lambdas/discover/providers/csv/` reads operator-uploaded CSVs from S3 (Bucket + Key on the Provider struct), parses header-row-with-name-column required, supports case-insensitive headers + ignored extras, skips blank-name rows, returns `DiscoveredBusiness` with Source="csv" + Confidence=1.0 (operator-supplied → fully trusted). Pure `parseCSV(io.Reader, cap)` is exposed for testing without S3 mocking. 10 csv tests + 1 ID test; full Go test sweep clean. CompaniesHouse + GooglePlaces providers land in the follow-up PR — same `Provider` interface, polite-fetch + cost-cap for Places.)*
