...
# Cognito Hosted UI fields — built from env vars set by the Deploy
...
if [[ -n "${COGNITO_HOSTED_UI_DOMAIN:-}" && -n "${COGNITO_APP_CLIENT_ID:-}" ]]; then
  ...
  COGNITO_AUTH_ORIGIN="${COGNITO_HOSTED_UI_DOMAIN}.auth.${AWS_REGION:-eu-west-2}.amazoncognito.com"
  COGNITO_REDIRECT_URI="${CALLBACK}"
  COGNITO_LOGIN_URL="https://${COGNITO_AUTH_ORIGIN}/login?client_id=${COGNITO_APP_CLIENT_ID}&response_type=code&scope=email+openid+profile&redirect_uri=${ENC_CALLBACK}"
fi
...
window.__FINANCE_CONFIG__ = {
  ...
  cognitoAuthOrigin: "${COGNITO_AUTH_ORIGIN}",
